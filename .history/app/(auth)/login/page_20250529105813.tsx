    ...
    <div className='flex flex-col'>
      ...
      <form className=" flex flex-col gap-2 mt-4" action="">
        ...
        <div className='w-xl p-2 flex items-center justify-between border px-3 border-gray-300 border-[1px] rounded-sm  bg-slate-100'>
          <input className=' w-xl border-none outline-none   ' type={!showPassword ? "password" : "text"} placeholder='Min 8 characters' />
          <div onClick={() => setShowPassword(!showPassword)}>{showPassword ? <Eye className='text-green-200' /> : <EyeOff />} </div>
        </div>
