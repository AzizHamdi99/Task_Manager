        ...
        <div className='p-6 flex flex-col gap-4'>
            ...
            <div className='grid grid-cols-3 gap-4 items-center'>
                {filteredTasks?.map((task, i) => {
                    ...
                    return (
                        <div key={i} className='bg-white p-4 shadow-md rounded-md flex flex-col gap-2'>
                            <div className='flex gap-4 items-center'>
                                <p className={task?.status === "Pending" ? "bg-[#f2e3fb] font-semibold text-[#8749bd] text-sm p-1 max-w-fit px-3 rounded-md" : task?.status === "In Progress" ? "bg-blue-300 text-blue-500 text-sm p-1 max-w-fit px-3 rounded-md" : "bg-[#c7f8da] text-[#448b5b] text-sm p-1 max-w-fit px-3 rounded-md"}>{task?.status}</p>
