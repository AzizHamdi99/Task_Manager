...
import { ClipboardCheck, Link, Paperclip } from 'lucide-react'
        ...
        <div className='p-6 flex flex-col gap-4'>
            ...
            <div className='grid grid-cols-3 gap-4 items-center '>
                {filteredTasks?.length > 0 ? (
                        ...
                        return (
                            ...
                            <div onClick={handleClick} key={i} className='bg-white p-4 shadow-md rounded-md flex flex-col gap-2 cursor-pointer hover:shadow-xl transition-all duration-300'>
                                <div className='flex gap-4 items-center'>
                                    <p className={task?.status === "Pending" ? "bg-[#f2e3fb] text-[#8749bd] text-sm p-1 max-w-fit px-3 rounded-md" : task?.status === "In Progress" ? "bg-blue-100 text-blue-800 text-sm p-1 max-w-fit px-3 rounded-md" : "bg-[#c7f8da] text-[#448b5b] text-sm p-1 max-w-fit px-3 rounded-md"}>{task?.status}</p>
                    ...
                    <div className="col-span-3 flex flex-col items-center justify-start text-center pt-10 bg-white rounded-md h-screen shadow">
                        <ClipboardCheck size={250} />
                        <p className="text-2xl font-bold text-gray-700 mt-6">No tasks found</p>
                        <p className="text-lg text-gray-500 mt-2">
                            You don't have any {selected.toLowerCase()} tasks.
                        </p>
                    </div>
                )}
